 ...
 * @date 2021/12/1
 */
public class MiddlewareDemoProcessor extends ProcessorHelper implements DemoProcessor {

    private static final Logger LOG = LoggerFactory.getLogger(MiddlewareDemoProcessor.class);

    private DemoSocketWrapper socketWrapper;

    public MiddlewareDemoProcessor(DemoSocketWrapper socketWrapper) {
        try {
    ...
    public boolean isReadEvent() {
        return socketWrapper.getSocketEventType() == SocketEventType.READ;
    }

    @Override
    public boolean isWriteEvent() {
        return socketWrapper.getSocketEventType() == SocketEventType.WRITE;
    }
    ...
    public void processReadEvent() throws Exception {
        ...
        DemoPoller poller = socketWrapper.getPoller();
        ...
        poller.getAttachmentQueue().offer(packetToSend);

        // Register the accepted socket channel to selector
        SelectionKey registeredKey = acceptedSocketChannel.register(selector, SelectionKey.OP_WRITE);
    ...
    public void processWriteEvent() throws Exception {
        ...
        // Retrieve the packet processed by server first
        Packet packetToSend = (Packet) needProcessEvent.getAttachmentForDataWrite();
