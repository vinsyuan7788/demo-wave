...
import com.demo.wave.fundamental.server.design.reactor.BaseServer;
import com.demo.wave.fundamental.server.design.reactor.poller.BasePoller;
import com.demo.wave.fundamental.server.design.reactor.poller.ReactorBasePoller;
 ...
 * @date 2021/11/29
 */
public class ReactorBaseAcceptor extends ContainerRunner implements Runnable, BaseAcceptor {

    private static final Logger LOG = LoggerFactory.getLogger(ReactorBaseAcceptor.class);

    private BaseServer server;
    private ContainerRunner acceptorThread;
    private BasePoller[] pollers;
    /**
    ...
    public ReactorBaseAcceptor(BaseServer server, int portToBind, int processingType) {
        try {
            this.server = server;
            this.server.setServerState(ServerState.INITIAL);

            acceptorThread = new ContainerRunner(this, "Acceptor");
            pollers = new ReactorBasePoller[numberOfPoller];
    ...
    public void startAcceptor() {
        try {
            for (int i = 0; i < pollers.length; i++) {
                pollers[i] = new ReactorBasePoller("Poller" + (i + 1), processingType);
